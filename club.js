...
async function openHeatmap(userId, displayName) {
    ...
    function drawCluster(trackSubset, x0, y0, cellW, cellH, label) {
        ...
        // [v2.30.1-p421] Heatmap · Fuego acumulativo. Los 3 pases mantienen
        // additive blending ('lighter' L6529) → donde múltiples tracks se cruzan
...
async function openUserProfile(userId, username, avatarUrl) {
    ...
    // [v2.30.1-p421] Cabecera B · Cinta dorada joyería 4px superior. Card blanca
    // de siempre + cinta metálica joyería con reflejo 4 tonos (mismo lenguaje que
    // las barras metálicas de biblio/plan/marcas). Border dorado subido a
    // rgba(196,136,30,.30) más marcado, halo intacto y shadow dorado adaptativo.
    ...
    hero.style.cssText = 'background:linear-gradient(135deg,var(--card) 0%,var(--surface) 100%);'
                      + 'border:1px solid rgba(196,136,30,.30);'
    ...
    // Row 2: stats con look del botón Wall dorado (gradient #c4881e→#e8a825).
    // [v2.30.1-p421] 3 pills doradas independientes con inset premium.
    // Número: negro en light / BLANCO en dark (Álvaro pidió texto blanco en dark).
    // Separadores verticales eliminados (redundantes con las pills separadas por gap).
    ...
    var _pillStyle = 'flex:1;text-align:center;background:linear-gradient(135deg,#c4881e,#e8a825);'
                   + 'border:1px solid rgba(0,0,0,.18);border-radius:9px;padding:6px 4px;'
